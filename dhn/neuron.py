...
class Neuron(nn.Module):
    ...
    def energy(self, g: torch.Tensor, x: torch.Tensor) -> torch.Tensor: #not sure what the type would be, scalar or a vector?
    ...
    def forward(self, x: torch.Tensor): #idk return type as I dont know return of energy
        g = self.activation(x)
        e_g =self.energy(g, x)
        return g, e_g
